...
impl ResourceServer {
    // Handler for the protected resource endpoint
    ...
    async fn get_resource(
        ...
        match grant {
            Ok(grant) => {
                info!("Access to protected resource granted for scope: {}", grant.scope);
            }
            Err(e) => {
                error!("Failed to access protected resource: {e:?}");
    ...
    pub async fn start(&self) {
        ...
        let app = axum::Router::new()
            .route("/resource", get(Self::get_resource))
            .with_state(state);
