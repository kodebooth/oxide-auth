...
impl AuthorizationServer {
    ...
    #[instrument(skip(state, solicitor))]
    ...
    async fn get_authorize(
        ...
        let Some(params) = request.body() else {
            error!("Missing query parameters in authorization request");
            return Err(WebError::Endpoint(OAuthError::BadRequest));
        };

        let Some(response_type) = params.unique_value("response_type") else {
            error!("Missing response_type in authorization request");
            return Err(WebError::Endpoint(OAuthError::BadRequest));
        };

        if response_type == "code" {
            info!("Processing 'Authorization Code Grant' request");
    ...
    async fn post_token(
        ...
        let Some(params) = request.body() else {
            error!("Missing body parameters in token request");
            return Err(WebError::Endpoint(OAuthError::BadRequest));
        };

        let Some(grant_type) = params.unique_value("grant_type") else {
            error!("Missing grant_type in token request");
            return Err(WebError::Endpoint(OAuthError::BadRequest));
        };

        if grant_type == "authorization_code" {
            info!("Processing 'Issuing an Access Token' request");
            return state
                .endpoint()
                .access_token_flow()
                .execute(request)
            ...
            info!("Processing 'Refreshing an Access Token' request");
            return state
                .endpoint()
                .refresh_flow()
                .execute(request)
        ...
        state
            ...
            .with_solicitor(FnSolicitor(|request: &mut OAuthRequest, _: Solicitation| {
                if request.query().is_some_and(|q| q.unique_value("allow").is_some()) {
                    info!("Resource owner granted consent");
                    OwnerConsent::Authorized("user".into())
                } else {
                    info!("Resource owner denied consent");
                    OwnerConsent::Denied
                }
            }))
            .authorization_flow()
