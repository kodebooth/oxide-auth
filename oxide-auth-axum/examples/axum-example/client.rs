...
impl Client {
    ...
    async fn home(State(state): State<ClientState>) -> Html<String> {
        let access_token = {
    ...
    async fn redirect(Query(query): Query<RedirectQuery>, State(state): State<ClientState>) -> Response {
    ...
    async fn refresh(State(state): State<ClientState>) -> impl IntoResponse {
